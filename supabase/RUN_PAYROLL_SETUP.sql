...
-- Fix: "Could not find the table 'public.payroll_adjustments' in the schema cache"
--
-- Cách làm:
-- 1. supabase.com/dashboard → chọn đúng project của khoespa.net.vn
-- 2. SQL Editor → New query
-- 3. Dán TOÀN BỘ file này → Run
-- 4. Đợi 30 giây → Ctrl+F5 trên app → mở module Lương lại
-- ============================================================

-- 1. Bảng điều chỉnh lương (một bảng duy nhất: thưởng, phạt, ứng, điều chỉnh…)
...
-- 4. Quyền + RLS (anon + authenticated)
...
-- 7. Kiểm tra — phải trả về 1 dòng "OK"
select 'OK — bảng payroll_adjustments đã sẵn sàng' as result;
