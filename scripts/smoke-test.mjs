...
test('commission policy: Gia Lai 1 = 40% giá vé thực thu, không tính Tips', () => {
  ...
  assert.equal(totals.serviceTotal, 450000)
  assert.equal(totals.serviceCommission, 180000)
  assert.equal(totals.commission, 180000)
  assert.equal(totals.tips, 100000)
  assert.equal(totals.total, 550000)
})
...
test('invoice payment flow: giá vé - khuyến mãi = thanh toán, thanh toán + tips = tổng KH', () => {
  const totals = calculateInvoiceTotals(
    ...
    100000,
    'vinh-long',
    [{ id: 'vip', name: 'Body VIP', price: 300000 }],
    'Vĩnh Long',
    '50000',
  )
...
test('invoice discount: 20% reduces revenue and commission, tips unchanged', () => {
  const totals = calculateInvoiceTotals(
    ...
    50000,
    'vinh-long',
    [{ id: 'body-vip', name: 'Body VIP', price: 300000 }],
    'Vĩnh Long',
    '20%',
  )
  assert.equal(totals.originalServiceTotal, 300000)
...
test('invoice discount: fixed amount 50000', () => {
  const totals = calculateInvoiceTotals(
    ...
    0,
    'vinh-long',
    [{ id: 'svc-a', name: 'Massage', price: 200000 }],
    'Vĩnh Long',
    '50000',
  )
...
test('invoice totals: duplicate services + tips', () => {
  ...
  const fallback = [
    { id: 'svc-a', name: 'Body 60', price: 200000 },
    { id: 'svc-a', name: 'Body 60', price: 200000 },
  ]
...
test('employee login: Cần Thơ / Gia Lai 1 / Gia Lai 2 credentials sync and password validation', async () => {
  ...
  const cn1 = await verifyLogin({
    ...
    branch: 'tram-spa',
    employeeId: 'tram-spa-thanh',
    password: computeEmployeeDefaultPassword('Thanh', getPasswordBranchName('tram-spa')),
  })
  assert.equal(cn1.ok, true, 'Cần Thơ (Trạm Spa) employee must login')
  assert.equal(cn1.user.employeeId, 'tram-spa-thanh')

  assert.equal(computeGiaLaiLoginPassword('Thu Diễm'), 'thudiemgialai')
  assert.equal(
    computeEmployeeDefaultPassword('Thu Diễm', getPasswordBranchName('gia-lai-1')),
    'thudiemgialai',
  )
  ...
  const { getEmployeeById, loadEmployees, EMPLOYEE_STATUS } = await import('../src/utils/employeeStorage.js')
  for (const name of GIA_LAI_EMPLOYEE_ROSTER['gia-lai-1']) {
    const employee = getEmployeeById(buildGiaLaiEmployeeId('gia-lai-1', name))
    assert.ok(employee, `Phải có nhân viên Gia Lai 1: ${name}`)
    assert.equal(employee.branchId, 'gia-lai-1')
    assert.equal(employee.status, EMPLOYEE_STATUS.ACTIVE)
  }

  const cn2 = await verifyLogin({
    role: ROLES.EMPLOYEE,
    branch: 'gia-lai-2',
    employeeId: buildGiaLaiEmployeeId('gia-lai-2', 'Bảo Ngọc'),
    password: 'baongocgialai',
  })
  assert.equal(cn2.ok, true, 'Gia Lai 2 employee must login')

  for (const name of GIA_LAI_EMPLOYEE_ROSTER['gia-lai-2']) {
    const employee = getEmployeeById(buildGiaLaiEmployeeId('gia-lai-2', name))
    assert.ok(employee, `Phải có nhân viên Gia Lai 2: ${name}`)
    assert.equal(employee.branchId, 'gia-lai-2')
  }
  ...
  assert.equal(wrongBranch.ok, false, 'Sai chi nhánh không đăng nhập được')

  const oldHuong = getEmployeeById('gia-lai-1-huong')
  if (oldHuong) {
    assert.notEqual(oldHuong.status, EMPLOYEE_STATUS.ACTIVE, 'NV cũ Hương phải bị khóa/resigned')
  }

  const otherBranch = loadEmployees().filter((e) => e.branchId === 'tram-spa' && e.status === EMPLOYEE_STATUS.ACTIVE)
  assert.ok(otherBranch.length > 0, 'Không được ảnh hưởng nhân viên chi nhánh khác')
...
test('invoice storage: save requires permission', async () => {
  clearCurrentUser()
...
test('invoice storage: branch manager can save scoped invoice', async () => {
  setSession({ role: ROLES.BRANCH_MANAGER, branch: 'vinh-long' })
...
test('invoice storage: employee create is forced to own branch/employee', async () => {
  setSession({ role: ROLES.EMPLOYEE, branch: 'vinh-long', employeeId: 'vinh-long-linh' })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  const result = await saveInvoice({
...
test('invoice storage: employee cannot edit another employee\'s invoice', async () => {
  setSession({ role: ROLES.BRANCH_MANAGER, branch: 'vinh-long' })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  await saveInvoice({
...
test('invoice storage: employee edit cannot reassign branch/employee via forged data', async () => {
  setSession({ role: ROLES.EMPLOYEE, branch: 'vinh-long', employeeId: 'vinh-long-linh' })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
...
test('invoice storage: admin sửa và xóa mọi hóa đơn', async () => {
  setSession({ role: ROLES.ADMIN, branch: ADMIN_BRANCH })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  await saveInvoice({
    id: 'admin-inv-1',
...
test('invoice storage: quản lý chi nhánh không xóa hoặc sửa hóa đơn đã lưu', async () => {
  setSession({ role: ROLES.BRANCH_MANAGER, branch: 'vinh-long' })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  await saveInvoice({
    id: 'manager-inv-1',
...
test('invoice storage: normalize customer phone on save', async () => {
  setSession({ role: ROLES.ADMIN, branch: ADMIN_BRANCH })
  const result = await saveInvoice({
    id: 'inv-phone-1',
    date: '2026-07-08',
...
test('canPermanentDeleteEmployee: chặn xóa khi đã có hóa đơn', async () => {
  setSession({ role: ROLES.ADMIN, branch: ADMIN_BRANCH })
  await saveInvoice({
    id: 'guard-inv-1',
    branchId: 'vinh-long',
...
test('deleteEmployee: chặn xóa vĩnh viễn khi có hóa đơn', async () => {
  setSession({ role: ROLES.ADMIN, branch: ADMIN_BRANCH })
  await saveInvoice({
    id: 'guard-inv-2',
    branchId: 'vinh-long',
...
test('getEmployeeLifetimeStats: tổng hợp doanh thu/tour/tips/hoa hồng/lương trọn đời', async () => {
  setSession({ role: ROLES.BRANCH_MANAGER, branch: 'vinh-long' })
  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  await saveInvoice({
    id: 'stats-inv-1',
...
test('report profit subtracts commission from ticket revenue, not tips', () => {
  ...
  assert.equal(report.summary.actualRevenue, 239000)
})
...
test('profit report: actualRevenue - totalSalary - expenses', async () => {
  const {
    computeActualRevenue,
    computeProfitAmount,
    computeProfitMarginPercent,
  } = await import('../src/utils/profitReport.js')
  ...
  assert.equal(actualRevenue, 11000000)
  assert.equal(profit, 4000000)
  assert.equal(computeProfitMarginPercent(actualRevenue, profit), 36.36)
  ...
  assert.equal(report.summary.tips, 10000)
  assert.equal(report.summary.actualRevenue, 210000)
...
test('expense types: Taxi available in catalog', async () => {
  const { EXPENSE_TYPES, EXPENSE_CATEGORY_CARDS, getExpenseTypeLabel } = await import('../src/constants/expenseTypes.js')
  assert.ok(EXPENSE_TYPES.some((item) => item.id === 'taxi'))
  assert.equal(getExpenseTypeLabel('taxi'), 'Taxi')
...
test('grouped service catalog: Gia Lai branches use 4-group UI', async () => {
  ...
  syncBranchCatalog(GIA_LAI_CN3_BRANCH_ID)
...
test('branch pricing isolation: mỗi chi nhánh có bảng giá riêng, không lẫn catalog', async () => {
  ...
  const { ensureServiceCatalogV2Migrated, setBranchDurationPrice } = await import('../src/utils/serviceCatalogV2Storage.js')
...
test('attendance repository: chỉ dùng bảng attendance', async () => {
  ...
  const setupPath = path.join(process.cwd(), 'supabase/RUN_ATTENDANCE_SETUP.sql')
...
test('employee profile policy: no date-based lock; invoice always allowed', async () => {
  const {
  ...
  assert.equal(getProfileComplianceFilterStatus(incomplete, '2025-12-31'), 'incomplete')
  assert.equal(getProfileComplianceFilterStatus(incomplete, '2026-07-05'), 'overdue')
  assert.equal(
    getEmployeeProfileBannerMessage(incomplete, '2025-12-31'),
    'Vui lòng hoàn thiện hồ sơ trước 10/07.',
  )
  assert.equal(isEmployeeProfileLocked(incomplete, '2026-07-05'), false)
  assert.equal(isEmployeeProfileLocked(incomplete, '2026-07-11'), false)
...
test('invoice duplicate guard: lần 3 cùng dữ liệu bị chặn', async () => {
  const { saveInvoice, loadInvoices, buildInvoiceDuplicateKey } = await import('../src/utils/invoiceStorage.js')

  localStorage.setItem('spa-manager-invoices', JSON.stringify([]))
  setSession({ role: ROLES.ADMIN, branch: ADMIN_BRANCH, name: 'Admin' })
  ...
  const first = await saveInvoice({ id: 'dup-1', ...base })
  assert.equal(first.success, true, first.error)
  const second = await saveInvoice({ id: 'dup-2', ...base })
  assert.equal(second.success, true, second.error)
  const third = await saveInvoice({ id: 'dup-3', ...base })
  assert.equal(third.success, false)
  assert.match(third.error, /nhập trùng/)
  assert.equal(loadInvoices().filter((inv) => buildInvoiceDuplicateKey(inv) === buildInvoiceDuplicateKey(base)).length, 2)
  clearCurrentUser()
})

test('getEmployeeProfileStatus: không yêu cầu chức vụ/ngày vào làm để Đầy đủ', () => {
...
test('branch employee match: canonical branch_id và nhân viên lương 0đ', async () => {
  const { employeeBelongsToBranch, isPayrollListEmployee } = await import('../src/utils/branchEmployeeMatch.js')
  const { mergeEmployeePayrollRows, aggregateBranchSummaries } = await import('../src/utils/payrollViewHelpers.js')
  const { EMPLOYEE_STATUS } = await import('../src/utils/employeeStorage.js')

  assert.equal(employeeBelongsToBranch({ branchId: 'gia-lai-3' }, 'gia-lai-2'), true)
  assert.equal(employeeBelongsToBranch({ branchId: 'soc-trang' }, 'gia-lai-2'), false)
  assert.equal(isPayrollListEmployee({ status: EMPLOYEE_STATUS.RESIGNED }, ''), false)
  assert.equal(isPayrollListEmployee({ status: EMPLOYEE_STATUS.RESIGNED }, EMPLOYEE_STATUS.RESIGNED), true)

  const branches = [{ id: 'tram-spa', name: 'Trạm Spa' }]
  const employees = [
    { id: 'e-zero', name: 'NV mới', branchId: 'tram-spa', status: 'active', position: 'KTV' },
    { id: 'e-busy', name: 'NV bận', branchId: 'tram-spa', status: 'active', position: 'LT' },
  ]
  const rows = [
    { employeeId: 'e-busy', branchId: 'tram-spa', ticketRevenue: 500000, commission: 100000, tips: 0, bonus: 0, penalty: 0, advance: 0, netSalary: 600000 },
  ]
  const merged = mergeEmployeePayrollRows(employees, rows, { branchId: 'tram-spa' })
  assert.equal(merged.length, 2)
  assert.equal(merged.find((r) => r.employeeId === 'e-zero')?.netSalary, 0)

  const summaries = aggregateBranchSummaries(branches, employees, rows)
  assert.equal(summaries[0].employeeCount, 2)
  assert.equal(summaries[0].netSalary, 600000)
})

test('branches: getCanonicalBranchesForDisplay trả về 8 chi nhánh', async () => {
  const { getCanonicalBranchesForDisplay } = await import('../src/utils/branchStorage.js')
  const { CANONICAL_BRANCH_IDS } = await import('../src/constants/canonicalBranches.js')
  const { getVisibleNavItems } = await import('../src/constants/auth.js')
  const { ROLES, ADMIN_BRANCH } = await import('../src/constants/roles.js')
  const { saveCurrentUser } = await import('../src/utils/authStorage.js')

  const branches = getCanonicalBranchesForDisplay()
  assert.equal(branches.length, CANONICAL_BRANCH_IDS.length)
  assert.ok(CANONICAL_BRANCH_IDS.every((id) => branches.some((b) => b.id === id)))

  saveCurrentUser({ role: ROLES.ADMIN, branch: ADMIN_BRANCH })
  assert.ok(getVisibleNavItems().some((item) => item.id === 'admin-branches'))
})
...
test('branch support invoice: totals, payroll, reports, branch gate', async () => {
  const { calculateBranchSupportTotals, isBranchSupportServiceEnabled, validateBranchSupportForm } = await import('../src/utils/branchSupportInvoice.js')
  const {
    BRANCH_SUPPORT_SERVICE_ID,
    BRANCH_SUPPORT_SERVICE_NAME,
    isBranchSupportInvoice,
  } = await import('../src/constants/branchSupportService.js')
  ...
  const totals = calculateBranchSupportTotals({
    ...
    tipsInput: '100000',
    supportNote: 'Hỗ trợ ca đông khách',
  })
  assert.equal(totals.serviceTotal, 500000)
  ...
  assert.equal(totals.services[0].id, BRANCH_SUPPORT_SERVICE_ID)
  assert.equal(totals.services[0].commissionPercent, 20)

  const errors = validateBranchSupportForm({
    branchId: 'soc-trang',
    employeeId: 'emp-1',
    priceInput: '500000',
    commissionRateInput: '20',
  ...
  const branchSupportInvoice = {
    ...
    branchId: 'soc-trang',
    employeeId: 'emp-1',
    customerName: 'Hỗ trợ chi nhánh',
    tips: 100000,
    commission: 100000,
    serviceTotal: 500000,
    total: 600000,
    serviceIds: [BRANCH_SUPPORT_SERVICE_ID],
    services: totals.services,
  }
  assert.equal(isBranchSupportInvoice(branchSupportInvoice), true)

  const employee = { id: 'emp-1', name: 'Lan', branchId: 'soc-trang', salaryRate: '0', position: 'KTV' }
  const payrollRow = computeEmployeePayrollRow(employee, [branchSupportInvoice], [], [])
  assert.equal(payrollRow.commission, 100000)
  assert.equal(payrollRow.tips, 100000)
  assert.equal(payrollRow.ticketRevenue, 500000)
  assert.equal(payrollRow.netSalary, 200000)

  const report = computeReportSummary([branchSupportInvoice])
  assert.equal(report.ticketRevenue, 500000)
  assert.equal(report.tips, 100000)
  assert.equal(report.commission, 100000)

  const regularTotals = calculateInvoiceTotals(
    ['combo-3'],
    '50000',
    'soc-trang',
    [{ id: 'combo-3', name: 'Combo 3', price: 399000 }],
    'Sóc Trăng',
    '',
  )
  assert.equal(regularTotals.tips, 50000)
  assert.ok(regularTotals.serviceTotal > 0)
