/**
 * Kiểm tra nhân viên Trạm Spa trên Supabase Production.
 * Chạy: node scripts/verify-payroll-from-production.mjs  (reuse env loader)
 * Hoặc: node scripts/inspect-tram-spa-employees.mjs
 */
...
async function loadProductionSupabaseEnv() {
  ...
  if (!url || !key) throw new Error('Production chưa nhúng Supabase URL/key hợp lệ.')
  return { url, key }
}

const { url, key } = await loadProductionSupabaseEnv()

const result = spawnSync('npx', ['vite-node', 'scripts/inspect-tram-spa-employees-core.mjs'], {
  cwd: ROOT,
